...
>
  ...
  <xsl:key
        name="RemoveFromOutput"
        match="wix:Component[ substring( wix:File/@Source, string-length( wix:File/@Source ) - 14 ) = 'PiWriteBack.exe' ]"
        use="@Id"
    />
  ...
  <!-- Don't render elements with the "RemoveFromOutput" key -->
  <xsl:template match="*[ self::wix:Component or self::wix:ComponentRef ][ key( 'RemoveFromOutput', @Id ) ]" />
